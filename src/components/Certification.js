...
import {certificate} from './ExperienceData';
...
const Certification = () => {
  ...
  return (
    <div className="experience">
      <h2 className='experience-section' >Certifications</h2>
      <div className="experience-blocks">
        ...
        <div className="experience-carousel" ref={carouselRef}>
          {certificate.map((certificate) => (
            <div 
             key={certificate.id}
             className={`experience-block projects ${certificate.title.toLowerCase().replace(' ', '-')}`}
              onClick={() => handleOpenModal(certificate.title, certificate.description, certificate.imageUrl)}
          >
            <img src={certificate.imageUrl} alt={certificate.title} className="experience-image" />
          </div>
        ))}
      </div>
