...
const Education = () => {
  ...
  return (
    <div className="experience">
      <h2 className='experience-section' >Education</h2>
      <div className="carousel-container">
        ...
        <div className="experience-carousel" ref={carouselRef}>
          {educationData.map((education) => (
              <div 
              key={education.id}
              className={`experience-block education ${education.title.toLowerCase().replace(' ', '-')}`}
              onClick={() => handleOpenModal(education.title, education.description, education.imageUrl)}
              >
                <img src={education.imageUrl} alt={education.title} className="experience-image" />
              </div>
            ))}
        </div>
